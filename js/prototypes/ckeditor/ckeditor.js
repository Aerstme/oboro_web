...
(function () {
			...
			on: function (b, d, g, k, h) {
				...
				if (0 > f.getListenerIndex(d)) {
					...
					for (var u = f.length - 1; 0 <= u; u--)
						if (f[u].priority <= h) return f.splice(u + 1, 0, p), {
							removeListener: r
						};
					f.unshift(p)
				}
			...
			enlarge: function (a, b) {
				...
				switch (a) {
										...
										if (t && CKEDITOR.dtd.$removeEmpty[q.getName()]) {
											...
											if (d.test(C)) q = null;
												...
												for (var D = q.$.getElementsByTagName("*"), M = 0, H; H =
													D[M++];)
													if (!CKEDITOR.dtd.$removeEmpty[H.nodeName.toLowerCase()]) {
														q = null;
														break
													}
											q && (A = !!C.length)
					...
					case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
						...
						var p, Q, P = CKEDITOR.dom.walker.blockBoundary(a == CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS ? {
								br: 1
							} : null),
							v = null,
							G = function (a) {
				...
				for (this._.nextNode = null; x;) {
					...
					if (F) x.type == CKEDITOR.NODE_TEXT && e.test(x.getText()) && (F = 0);
						...
						if (CKEDITOR.dtd.$block[q] && "false" == x.getAttribute("contenteditable")) {
							...
							b(this, a, d);
							break
						} else if (x.isBlockBoundary(this.forceBrBreak && !w && {
								br: 1
							})) {
							if ("br" == q) F = 1;
					...
					if (k && !J)
						for (; !x.getNext(h) && !u;) {
							q = x.getParent();
							if (q.isBlockBoundary(this.forceBrBreak && !w && {
									br: 1
								})) {
								J = 1;
								F = 0;
	...
	CKEDITOR.config = {
		customConfig: "js/prototypes/ckeditor/config.js",
		autoUpdateElement: !0,
		language: "",
		defaultLanguage: "en",
		...
		fullPage: !1,
		height: 200,
		contentsCss: CKEDITOR.getUrl("js/prototypes/ckeditor/contents.css"),
		extraPlugins: "",
		removePlugins: "",
		protectedSource: [],
		tabIndex: 0,
	...
	(function () {
		function a(a, b, c, d, l) {
			...
			for (e in b) {
				q = b[e];
				q = "boolean" == typeof q ? {} : "function" == typeof q ? {
					match: q
				} : M(q);
		...
		function l(a, b, c) {
			...
			if (D.$empty[d] || !a.children.length) "hr" == d && "br" == b ? a.replaceWith(w()) : (a.parent && c.push({
				...
				if ("br" == b) a.previous && !F(a.previous) && (b = w(), b.insertBefore(a)), a.next && !F(a.next) && (b = w(), b.insertAfter(a)), a.replaceWithChildren();
					...
					if (l) a.name = b, a.attributes = {}, c.push({
						check: "parent-down",
						el: a
					});
					else {
	...
	(function () {
		CKEDITOR.lang = {
			...
			load: function (a, d, b) {
				...
				this[a] ? d() : CKEDITOR.scriptLoader.load(CKEDITOR.getUrl("js/prototypes/ckeditor/lang/" + a + ".js"), d, this)
			},
			detect: function (a, d) {
	...
	CKEDITOR.plugins = new CKEDITOR.resourceManager("js/prototypes/ckeditor/plugins/", "plugin");
		...
		function B(a) {
			...
			CKEDITOR.plugins.load(c.split(","), function (c) {
				...
				for (var f in c) {
					...
					if (p && (r = p.match(q)))
						for (; p = r.pop();) CKEDITOR.error("editor-plugin-required", {
							plugin: p.replace(",", ""),
							requiredBy: f
						});
...
""], [/^data-cke-.*/, ""], ["hidefocus", ""]],
		...
		m = function () {
			...
			function b(c, d) {
				...
				for (var k = p[k.getName()], h = 0, C = c.getChildren(), A = C.count(), D = -1, m = -1, r = 0, H = e.contains(p.$list); h < A; ++h) e = C.getItem(h), a(e) ? (n = e.getName(), H && n in CKEDITOR.dtd.$list ? g = g.concat(b(e, d)) : (t = !!k[n],
					...
					node: e,
					allowed: 1
				}); - 1 < D && (g[D].firstNotAllowed = 1); - 1 < m && (g[m].lastNotAllowed = 1);
				return g
			}
		...
		getStylesSet: function (a) {
			if (this._.stylesDefinitions) a(this._.stylesDefinitions);
				...
				if (!1 === b) a(null);
					...
					var b = b.split(":"),
						c = b[0];
					CKEDITOR.stylesSet.addExternal(c, b[1] ? b.slice(1).join(":") : CKEDITOR.getUrl("js/prototypes/ckeditor/styles.js"), "");
	...
	(function () {
		function a() {
			return CKEDITOR.getUrl(CKEDITOR.skinName.split(",")[1] || "js/prototypes/ckeditor/skins/" + CKEDITOR.skinName.split(",")[0] + "/")
		}
...
' tabIndex\x3d"-1" hidefocus\x3d"true" role\x3d"tab"\x3e', a.label, "\x3c/a\x3e"].join(""));
	...
	(function () {
		...
		function z(b) {
			...
			function w() {
				if ("paste" == m.mainPasteEvent) return b.fire("beforePaste", {
					type: "auto",
					method: "paste"
				}), !1;
				b.focus();
